...
public class FreemarkerService {
	...
	public boolean build(String templatePath, Map<String, Object> data, String htmlPath, String htmlName) {
		...
		try (final Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(htmlFile), "UTF-8"))) {
	...
	public String templateConvert(String content, Map<String, Object> data) {
		...
		try (final Writer writer = new StringWriter()) {
			final Template template = this.configuration.getTemplate(TEMPLATE, "UTF-8");
			template.process(data, writer);
